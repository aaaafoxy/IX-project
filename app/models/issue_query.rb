...
class IssueQuery < Query
  ...
  def sql_for_assigned_to_id_field(field, operator, value)
    ...
    case operator
    when '='
      "(issues.assigned_to_id IN (#{ids}) OR issues.id IN (SELECT issue_id FROM issue_assignees WHERE user_id IN (#{ids})))"
    when '!'
      "(issues.assigned_to_id NOT IN (#{ids}) AND issues.id NOT IN (SELECT issue_id FROM issue_assignees WHERE user_id IN (#{ids})))"
    else
