...
class IssuesController < ApplicationController
  ...
  def edit
    unless @issue.editable_by_multi_assignee?(User.current)
      deny_access
      return
    end
  ...
  def update_issue_from_params
    ...
    if params[:issue] && params[:issue][:assignee_ids]
      @issue.assignee_ids = params[:issue][:assignee_ids].reject(&:blank?)
      @issue.assigned_to_id = @issue.assignee_ids.first if @issue.assignee_ids.present?
    end
    @priorities = IssuePriority.active
    @allowed_statuses = @issue.new_statuses_allowed_to(User.current)
    true
